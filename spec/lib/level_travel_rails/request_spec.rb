...
RSpec.describe LevelTravelRails::Request do
  describe '#get' do
    ...
    before do
      LevelTravelRails.api_token = api_token
  ...
  describe '#post' do
    subject(:request) { described_class.post(path, payload) }

    let(:path) { '/foo/bar' }
    let(:payload) { { foo: 1, bar: 10 } }

    let(:expected_response) { response.transform_keys(&:to_sym) }
    let(:api_token) { 'api_token' }
    let(:request_uri) { described_class::HOST + path }
    let(:headers) do
      {
        Accept: 'application/vnd.leveltravel.v3',
        Authorization: format('Token token="%<api_token>s"', api_token: api_token)
    ...
    before do
      ...
      stub_request(:post, request_uri).with(headers: headers).to_return(
        body: Oj.dump(response),
        status: status,
        headers: { 'content-type' => 'application/json; charset=utf-8' }
      )
